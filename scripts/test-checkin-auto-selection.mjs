...
assert.match(checkin, /oneClick\.addEventListener\('click',[\s\S]*?DailyCheckinPage\.getState\(\)[\s\S]*?DailyCheckinPage\.isDefaultSelected[\s\S]*?await waitForCheckinSubmit\(\)[\s\S]*?submit\.click\(\)/, 'toolbar oneClick must skip a selected mood before looking up submit');
